...
const MobileNav = ({ closeNav, nav }: Props) => {
  ...
  return (
    <div
      className={`fixed ${navStyle} right-0 left-0 bottom-0 top-0 transition-all duration-500 bg-[#000000e0]`}
    >
      <ImCross
        onClick={closeNav}
        className="absolute top-[2rem] w-[1.5rem] h-[1.5rem] right-[2rem] text-white"
      />
      <div className="bg-red-600 transition-all duration-500 delay-200 flex items-center justify-center w-[70%] h-[100%]">
        {/* nav links */}
